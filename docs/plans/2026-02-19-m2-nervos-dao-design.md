...
## Prerequisite: DAO Type Script Registration

The light client only indexes cells for scripts registered via `nativeSetScripts`. Currently only the user's lock script is registered (`GatewayRepository.registerAccount()` line 386-398). The DAO type script must also be registered for `nativeGetCells` to return DAO cells.

**Solution:** Add `registerDaoTypeScript()` using `CMD_SET_SCRIPTS_PARTIAL` (adds without replacing lock script). Call after `registerAccount()` completes. Update all `nativeSetScripts(CMD_SET_SCRIPTS_ALL)` call sites (lines 398, 521, 707) to include the DAO type script in the list so it's never wiped.
