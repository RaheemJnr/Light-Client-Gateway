...
## Feature 0: DAO Type Script Registration (Prerequisite)

### Overview

The CKB light client only indexes cells for scripts explicitly registered via `nativeSetScripts`. Currently, only the user's lock script is registered (in `registerAccount()` at `GatewayRepository.kt:386-398`, using `scriptType = "lock"`). The light client will **not** index DAO cells unless the DAO type script is also registered.

Without this, `nativeGetCells` with `script_type: "type"` and the DAO type script would return zero results — the light client hasn't been watching for those cells.

### Solution

Add a `registerDaoTypeScript()` function in `GatewayRepository` that registers the DAO type script alongside the user's lock script. Uses `CMD_SET_SCRIPTS_PARTIAL` to add without replacing the existing lock script registration.

### Files to Modify

#### `GatewayRepository.kt` (add after `registerAccount()` at ~line 410)

**`registerDaoTypeScript(blockNumber: String): Result<Unit>`**
- Build a `JniScriptStatus` with:
  - `script` = DAO type script (code_hash = `DaoConstants.DAO_CODE_HASH`, hash_type = `DaoConstants.DAO_HASH_TYPE`, args = `DaoConstants.DAO_ARGS`)
  - `scriptType` = `"type"`
  - `blockNumber` = same block number used for the lock script registration
- Call `nativeSetScripts(json, CMD_SET_SCRIPTS_PARTIAL)` — partial add, does not replace the lock script

**Call site:** invoke `registerDaoTypeScript()` at the end of `registerAccount()` (after line 398), using the same `finalBlockNum`. This ensures DAO indexing starts from the same sync point as the wallet.

Also call it in the other `nativeSetScripts` call sites:
- `GatewayRepository.kt:521` (post-send re-registration)
- `GatewayRepository.kt:707` (post-send re-registration)

These use `CMD_SET_SCRIPTS_ALL` which would wipe the DAO type script. Either:
- (a) Change those to also include the DAO type script in the list, or
- (b) Follow each `CMD_SET_SCRIPTS_ALL` call with a `CMD_SET_SCRIPTS_PARTIAL` add for the DAO script
...
| Day | Task |
|-----|------|
| 1 | Create `DaoConstants.kt` and `DaoModels.kt` — all data models, enums, status determination logic. Implement `registerDaoTypeScript()` in `GatewayRepository` and update all `nativeSetScripts` call sites to preserve DAO type script |
...
| `GatewayRepository.kt:~398` | Add `registerDaoTypeScript()`, update all `nativeSetScripts` call sites (lines 398, 521, 707) to preserve DAO type script |
