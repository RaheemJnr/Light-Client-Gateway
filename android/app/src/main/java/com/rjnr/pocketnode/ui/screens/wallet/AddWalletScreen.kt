        ...
        Column(
            ...
            when (selectedMode) {
                0 -> {
                    ...
                    if (uiState.parentWallets.isNotEmpty()) {
                        OptionCard(
                            title = "HD Sub-Account",
                            description = "Derive from an existing mnemonic wallet",
...
private fun SubAccountForm(uiState: AddWalletUiState, viewModel: AddWalletViewModel) {
    ...
    val selectedParent = uiState.parentWallets.find { it.walletId == uiState.selectedParentId }
