...
class HomeViewModel @Inject constructor(
    ...
    private suspend fun checkSyncStatusAndRefresh() {
        ...
        repository.getAccountStatus()
            .onSuccess { status ->
                ...
                if (!status.isSynced) {
                    Log.d(TAG, "Account still syncing (${(status.syncProgress * 100).toInt()}%), will poll for updates")
    ...
    private fun startSyncPolling() {
        ...
        syncPollingJob = viewModelScope.launch {
            var attempts = 0
            val maxAttempts = 120 // Poll for up to 10 minutes
            ...
            while (attempts < maxAttempts) {
                ...
                repository.getAccountStatus()
                    .onSuccess { status ->
                        ...
                        if (status.isSynced) {
                            Log.d(TAG, "Account fully synced!")
                            refresh()
                            return@launch
                        }
                    ...
                    .onFailure {
                        Log.e(TAG, "Failed to check sync status", it)
                    }
            }

            Log.w(TAG, "Sync polling timed out")
